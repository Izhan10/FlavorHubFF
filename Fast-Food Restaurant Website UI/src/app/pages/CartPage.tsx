            ...
            <div className="bg-white rounded-2xl p-6 shadow-md sticky top-24">
              ...
              <button
                onClick={() => {
                  ...
                  const orderItems = cart.map(item => `- ${item.name} x${item.quantity} ($${(item.price * item.quantity).toFixed(2)})`).join('%0A');
                  const subtotal = getCartTotal().toFixed(2);
                  const tax = (getCartTotal() * 0.08).toFixed(2);
                  const total = (getCartTotal() + 2.99 + getCartTotal() * 0.08).toFixed(2);

                  const message = `*New Order!* %0A%0A${orderItems}%0A%0A*Subtotal:* $${subtotal}%0A*Tax:* $${tax}%0A*Delivery:* $2.99%0A*Total:* $${total}`;

                  window.open(`https://wa.me/${phoneNumber}?text=${message}`, '_blank');
                }}
                className="w-full bg-brand-red hover:bg-red-700 text-white py-4 rounded-full font-bold text-lg flex items-center justify-center gap-2 transition-colors duration-300 mb-4"
              >
                Proceed to Checkout
                <ArrowRight className="w-5 h-5" />
              </button>
